...
// カート内の商品数量・希望価格を変更（ユーザー操作用）
export async function updateCartItem(itemId: string, quantity: number, desiredPrice?: number | null) {
    const supabase = await createClient()
    ...
    if (desiredPrice !== undefined) {
        updateData.desired_price = desiredPrice
    }
