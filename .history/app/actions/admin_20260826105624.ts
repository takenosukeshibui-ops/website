...
export async function updateItemStatus(itemId: string, status: string) {
    ...
    if (orderId) {
        ...
        // 決済待ち・発送完了・キャンセルの場合はステータスを巻き戻さない
        ...
        if (!isAdvancedStatus) {
            ...
            if (itemLinks && itemLinks.length > 0) {
                ...
                if (items && items.length > 0) {
                    ...
                    if (orderUpdateError) {
                        console.error('注文ステータス更新失敗:', orderUpdateError.message)
                    }
                }
            }
        }
    ...
    // admin.ts の 判定部分の手前に挿入
console.log('取得した全商品のステータス配列:', statuses);
console.log('有効な商品のステータス配列:', activeStatuses);
