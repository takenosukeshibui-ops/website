...
// Wise送信完了の取消（ステータスをpendingに戻し、自動計算に委ねる）
export async function cancelPaymentRequired(orderId: string) {
    ...
    if (error) {
        console.error('送信取消失敗:', error.message)
        throw new Error(error.message)
    }
...
export async function deleteShip(orderId: string) {
    ...
    const { error } = await supabase
        ...
        .update({
            ...
            status: 'payment_required' // ← 追跡番号削除時は「決済待ち」に戻す
        })
        .eq('id', orderId)
...
export async function updateItemStatus(itemId: string, status: string) {
    ...
    if (error) {
        console.error('商品ステータス更新失敗:', error.message)
        throw new Error(error.message)
    }
