...
// prevState を第一引数に追加し、エラー/成功のオブジェクトを返すように変更
export async function updateProfile(prevState: any, formData: FormData) {
    ...
    // 【追加】PayPalかWise、どちらかが入力されているかチェック
    if (!paypal_email && !wise_email) {
        return { error: 'PayPalまたはWiseのどちらかの決済用メールアドレスを入力してください。' }
    }
    ...
    // 成功メッセージを返す（今回はリダイレクトせず、画面に「保存しました」と出します）
    return { success: true }
}