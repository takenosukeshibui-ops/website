...
// ★ useState と useEffect を追加でインポート
...
export default function ProfileForm({ profile, userEmail }: { profile: any, userEmail: string }) {
    ...
    // ★ ポップアップの表示状態を管理するステート
    const [showToast, setShowToast] = useState(false)

    // ★ エラーや成功状態が変わった時にポップアップを表示し、5秒後に消す処理
    useEffect(() => {
    ...
    return (
        <>
            {/* ★ showToast が true の時だけ表示。背景を半透明 (bg-opacity-90) とすりガラス効果 (backdrop-blur-sm) に変更 */}
            {showToast && (state?.error || state?.success) && (
                <div className="fixed bottom-24 left-1/2 transform -translate-x-1/2 z-50 animate-fade-in-up transition-opacity duration-300">
                    {state?.error && (
                        <div className="px-6 py-3 bg-red-600/90 backdrop-blur-sm text-white text-sm font-bold rounded-full shadow-lg flex items-center gap-2">
                            ⚠️ {state.error}
                        </div>
                    )}
                    {state?.success && (
                        <div className="px-6 py-3 bg-emerald-600/90 backdrop-blur-sm text-white text-sm font-bold rounded-full shadow-lg flex items-center gap-2">
                            ✅ 設定を保存しました
                        </div>
                    )}
                </div>
