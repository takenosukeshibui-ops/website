...
export default function ProfileForm({ profile, userEmail }: { profile: any, userEmail: string }) {
    ...
    return (
        <>
            {/* ★ 確実に半透明になるよう bg-opacity-80 と backdrop-blur-md を指定 */}
            {showToast && (state?.error || state?.success) && (
                <div className="fixed bottom-24 left-1/2 transform -translate-x-1/2 z-50 animate-fade-in-up transition-opacity duration-300">
                    {state?.error && (
                        <div className="px-6 py-3 bg-red-600 bg-opacity-80 backdrop-blur-md text-white text-sm font-bold rounded-full shadow-lg flex items-center gap-2">
                            ⚠️ {state.error}
                        </div>
                    )}
                    {state?.success && (
                        <div className="px-6 py-3 bg-emerald-600 bg-opacity-80 backdrop-blur-md text-white text-sm font-bold rounded-full shadow-lg flex items-center gap-2">
                            ✅ 設定を保存しました
                        </div>
                    )}
                </div>
