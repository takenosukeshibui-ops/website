    ...
    return (
        <div className="flex min-h-screen items-center justify-center bg-slate-50">
            <form action={action} className="w-full max-w-sm rounded-lg bg-white p-8 border border-slate-200 shadow-sm">
                <h1 className="mb-6 text-xl font-bold text-slate-900 text-center">新規アカウント登録</h1>
                ...
                <div className="mb-6">
                    <label className="block text-xs font-bold text-slate-600 mb-1">パスワード (6文字以上)</label>
                ...
                <button disabled={pending} className="w-full rounded bg-blue-600 p-2 text-white font-bold hover:bg-blue-700 disabled:opacity-50 transition-colors">
                    {pending ? '登録処理中...' : 'アカウントを作成'}
                </button>

                <div className="mt-6 text-center border-t border-slate-100 pt-4">
                    <p className="text-xs text-slate-500">すでにアカウントをお持ちですか？</p>
                    <Link href="/login" className="text-sm font-bold text-slate-600 hover:text-slate-900 hover:underline mt-1 inline-block">
                        ログイン画面へ戻る
                    </Link>
                </div>
            </form>
