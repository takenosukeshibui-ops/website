...
export default function ClientAdminPage({ orders: initialOrders }: { orders: any[] }) {
    ...
    const calculateOrderStatus = (order: any) => {
        // 追跡番号が存在し、ステータスがshippedの場合のみ『国際発送済み』
        if (order.status === 'shipped' || (order.tracking_number && String(order.tracking_number).trim() !== '' && order.status === 'shipped')) {
            return 'shipped'
        }
    ...
    // 「発送完了」ボタンが押された時のみステータスを shipped に変更
        ...
        setOrders((prevOrders) =>
            prevOrders.map((order) => {
                ...
                return {
                    ...
                    status: 'shipped', // ボタン押下で即座に発送済みに指定
                }
            })
        )
                                    ...
                                    <td className="border border-slate-300 p-2">
                                        {hasTracking && order.status === 'shipped' ? (
                                            ...
                                            <form action={handleShip.bind(null, order.id)} className="flex items-center gap-1.5">
                                                ...
                                                />
                                                <SubmitButton pendingText="...">発送完了</SubmitButton>
                                            </form>
                                        )}
                                    </td>
                                </tr>
