...
export default function ClientAdminPage({ orders: initialOrders }: { orders: any[] }) {
    ...
    // 編集モードを保持するステート
    const [editingInvoiceOrderIds, setEditingInvoiceOrderIds] = useState<Set<string>>(new Set())
    const [editingShipOrderIds, setEditingShipOrderIds] = useState<Set<string>>(new Set())
    ...
    // 編集モード切り替え
    const toggleInvoiceEdit = (orderId: string) => {
        setEditingInvoiceOrderIds((prev) => {
            const next = new Set(prev)
            if (next.has(orderId)) next.delete(orderId)
            else next.add(orderId)
            return next
    ...
    const toggleShipEdit = (orderId: string) => {
        setEditingShipOrderIds((prev) => {
            const next = new Set(prev)
            if (next.has(orderId)) next.delete(orderId)
            else next.add(orderId)
            return next
    ...
    const handleItemStatusUpdated = (orderId: string, itemId: string, newStatus: string) => {
        setOrders((prevOrders) =>
            prevOrders.map((order) => {
                ...
                const allStatuses = updatedOrderItems.map((oi: any) => {
                    const itemObj = Array.isArray(oi.items) ? oi.items[0] : oi.items
                    return itemObj?.status
                }).filter(Boolean)

                let calculatedOrderStatus = order.status

                if (!['payment_required', 'shipped'].includes(order.status)) {
                    if (allStatuses.length > 0) {
                        if (allStatuses.every((s: string) => ['out_of_stock', 'cancelled'].includes(s))) {
                            calculatedOrderStatus = 'cancelled'
                        } else if (allStatuses.every((s: string) => s === 'pending')) {
                            calculatedOrderStatus = 'pending'
                        } else {
                            ...
                            if (activeStatuses.length > 0) {
                                if (activeStatuses.every((s: string) => s === 'in_warehouse')) {
                                    calculatedOrderStatus = 'calculating'
                                } else if (activeStatuses.every((s: string) => s === 'purchased')) {
                                    calculatedOrderStatus = 'procured'
            ...
            <table className="min-w-full border-collapse border border-slate-300 text-sm text-left bg-white shadow-sm">
                <thead className="bg-slate-100">
                    <tr>
                        ...
                        <th className="border border-slate-300 p-2 min-w-[320px]">請求書発行</th>
                        <th className="border border-slate-300 p-2 min-w-[240px]">追跡番号追加</th>
                    </tr>
                </thead>
                <tbody>
                    {orders?.map((order) => {
                        ...
                        // 既に発行済み/発送済みかの判定
                        const hasInvoice = order.total_amount !== null && order.total_amount !== undefined && order.total_amount !== 0
                        const isEditingInvoice = editingInvoiceOrderIds.has(order.id)

                        const hasTracking = Boolean(order.tracking_number)
                        const isEditingShip = editingShipOrderIds.has(order.id)
                                    ...
                                    <td className="border border-slate-300 p-2">
                                        {hasInvoice && !isEditingInvoice ? (
                                            <div className="flex items-center justify-between gap-2 bg-amber-50 p-1.5 rounded border border-amber-200">
                                                ...
                                                <button
                                                    type="button"
                                                    onClick={() => toggleInvoiceEdit(order.id)}
                                    ...
                                    <td className="border border-slate-300 p-2">
                                        {hasTracking && !isEditingShip ? (
                                            <div className="flex items-center justify-between gap-2 bg-emerald-50 p-1.5 rounded border border-emerald-200">
                                                ...
                                                <button
                                                    type="button"
                                                    onClick={() => toggleShipEdit(order.id)}
                                                    className="text-[10px] text-blue-600 hover:underline whitespace-nowrap font-medium"
                                                >
                                                    変更
                                                </button>
                                            ...
                                            <form action={handleShip.bind(null, order.id)} className="flex items-center gap-1.5">
                                                <input
                                                    name="trackingNumber"
                                                    placeholder="追跡番号"
                                                    ...
                                                    className="border border-slate-300 p-1 rounded text-xs w-28"
                                                />
                                                <SubmitButton pendingText="...">
                                                    {hasTracking ? '再更新' : '発送完了'}
                                                </SubmitButton>
                                                {isEditingShip && (
                                                    <button
                                                        type="button"
