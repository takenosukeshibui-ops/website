...
// ※展開エリア内の「購入価格(確定)」等で使用する入力用コンポーネント
...
// 請求明細を自動計算する関数
function calculateInvoiceDetails(order: any, weight: number, hasInvoice: boolean) {
    ...
    // =========================================================
    let shippingFee = 0;
    if (hasInvoice && order.shipping_fee !== null && order.shipping_fee !== undefined) {
        // すでに保存済みの場合はデータベースの値を優先
    ...
    // 表示の整合性を保つため、保存済みならDBの金額を優先
    if (hasInvoice && order.total_amount) {
            ...
            <table className="min-w-full border-collapse border border-slate-300 text-sm text-left bg-white shadow-sm">
                ...
                <tbody>
                    {orders?.map((order) => {
                        ...
                        const details = calculateInvoiceDetails(order, weightVal, hasInvoice)
                        ...
                        return (
                            <React.Fragment key={order.id}>
                                <tr className="hover:bg-slate-50 transition-colors border-t border-slate-300">
                                    ...
                                    <td className="border border-slate-300 p-2 text-center">
                                        {hasInvoice ? (
                                            <span className="inline-block bg-slate-100 text-slate-500 border border-slate-200 text-[10px] px-2 py-1 rounded font-bold">
                                                保存済
                                            </span>
                                        ) : (
                                    ...
                                    <td className="border border-slate-300 p-2 text-right">
                                        {hasInvoice ? (
                                            <div className="flex items-center justify-between gap-2 bg-amber-50 p-1.5 rounded border border-amber-200">
                                                <span className="font-bold text-amber-700 text-sm ml-1">
                                                    {(order.total_amount || 0).toLocaleString()} 円
                                                </span>
                                                <button
                                                    type="button"
                                                    onClick={() => handleDeleteInvoice(order.id)}
                                                    className="text-[10px] text-rose-600 hover:underline font-medium px-1"
                                                >
                                                    削除
                                                </button>
                                            </div>
                                        ) : (
                                            <span className="font-bold text-slate-600 text-sm block pr-2">
                                                {details.grandTotal.toLocaleString()} 円
                                            </span>
                                        )}
                                    </td>

                                    {/* ★ 請求書送信 (送信条件: 総重量の保存が行われ、hasInvoiceがtrueであること) */}
                                            ...
                                            <div className="bg-white border border-slate-200 rounded shadow-inner p-4 mb-4">
                                                ...
                                                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                                                    ...
                                                    <div className="flex flex-col">
                                                        ...
                                                        <span className="text-sm font-mono text-blue-600 font-bold">{hasInvoice ? '-' : (weightVal > 0 ? `${weightVal} kg` : '-')}</span>
                                                    </div>
                                                    <div className="flex flex-col">
