...
// Wise の請求用ディープリンク作成ボタン
function WiseInvoiceLinkButton({ order }: { order: any }) {
    const wiseEmail = order.profiles?.wise_email || ''
    const amount = order.total_amount || 0

    // ★ご自身の Wise ビジネスハンドル名（または指定の支払いID）を入力してください
    const wiseHandle = 'your_business_handle'
    ...
    const handleOpenWise = () => {
        ...
        // Wise の画面へ金額・メールアドレス・説明文を渡して別タブで開く
