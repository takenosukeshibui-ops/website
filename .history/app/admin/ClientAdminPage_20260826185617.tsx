...
// Wise 送金テスト/実行用ボタンコンポーネント
function WisePayoutButton({ order }: { order: any }) {
    const [loading, setLoading] = useState(false)

    const handlePayout = async () => {
        const wiseEmail = order.profiles?.wise_email
        if (!wiseEmail) {
            alert('このユーザーは Wise 用メールアドレスを登録していません。')
            return
        }

        const payoutAmount = order.total_amount || 1000 // デフォルト1,000円（または請求合計額）

        if (!confirm(`注文番号 #${order.order_number ?? ''}\nユーザー: ${order.profiles?.full_name || '未設定'}\nWise宛先: ${wiseEmail}\n\n金額 ${payoutAmount.toLocaleString()} 円 のWise送金を実行しますか？`)) {
            return
        }

        setLoading(true)
        try {
            const result = await executeWisePayout(order.user_id, payoutAmount)
            if (result.success) {
                alert(`✅ Wise送金成功！\nTransfer ID: ${result.data?.transferId}`)
            } else {
                alert(`❌ Wise送金失敗: ${result.error}`)
            }
        } catch (err: any) {
    ...
    return (
        <button
            ...
            disabled={loading}
            className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white text-xs font-bold rounded shadow-sm transition-colors whitespace-nowrap"
        >
            {loading ? '処理中...' : 'Wise送金'}
        </button>
    )
            ...
            <table className="min-w-full border-collapse border border-slate-300 text-sm text-left bg-white shadow-sm">
                <thead className="bg-slate-100">
                    <tr>
                        ...
                        {/* ★ Wise 送金用のヘッダー列を追加 */}
                        <th className="border border-slate-300 p-2 whitespace-nowrap text-center">Wise 送金</th>
                    </tr>
                </thead>
                <tbody>
                    {orders?.map((order) => {
