...
export default function ClientAdminPage({ orders }: { orders: any[] }) {
    ...
    const handleInvoice = async (orderId: string, formData: FormData) => {
        ...
        await sendInvoice(orderId, shippingFee, totalAmount)[cite: 8]
    }

    // 発送完了ハンドラ[cite: 8]
    const handleShip = async (orderId: string, formData: FormData) => {
        const trackingNumber = formData.get('trackingNumber') as string || ''
        await shipOrder(orderId, trackingNumber)[cite: 8]
    }
            ...
            <table className="min-w-full border-collapse border border-slate-300 text-sm text-left bg-white shadow-sm">
                <thead className="bg-slate-100">
                    <tr>
                        ...
                        <th className="border border-slate-300 p-2 min-w-[200px]">請求書発行</th>
                ...
                <tbody>
                    {orders.map((order) => {
                        ...
                        return (
                            <>
                                {/* 注文情報の親行 */}
                                <tr key={order.id} className="hover:bg-slate-50 transition-colors border-t border-slate-300">
                                    ...
                                    <td className="border border-slate-300 p-2">
                                        <form action={handleInvoice.bind(null, order.id)} className="flex items-center gap-2">
                                            <input
                                                ...
                                                className="border border-slate-300 p-1 rounded text-xs w-20"
                                            />
                                            <input
                                                name="totalAmount"
                                                type="number"
                                                ...
                                                className="border border-slate-300 p-1 rounded text-xs w-24"
                                            />
                                ...
                                {isExpanded && (
                                    <tr key={`${order.id}-items`} className="bg-slate-50/70">
                                                ...
                                                <table className="min-w-full border-collapse text-xs text-left">
                                                    <thead>
                                                        <tr className="border-b border-slate-200 bg-slate-100 text-slate-700">
                                                            ...
                                                            <th className="p-2 font-semibold w-28">価格 (円)</th>
