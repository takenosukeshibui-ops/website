                                                ...
                                                <table className="min-w-full border-collapse border border-slate-200 text-xs text-left">
                                                    ...
                                                    <tbody>
                                                        {order.order_items?.map((oi: any, idx: number) => {
                                                            ...
                                                            // ステータスが info_required なら「送信済み(返信待ち)」
                                                            const isNoteSent = item.status === 'info_required' && item.admin_note
                                                            // ステータスが info_required 以外に戻った（＝返信が来た）が過去の連絡内容が残っている場合
                                                            const hasPastNote = item.status !== 'info_required' && item.admin_note
                                                                    ...
                                                                    <td className="border border-slate-200 p-2">
                                                                        <form action={async (formData) => {
                                                                            ...
                                                                            <div className="flex flex-col gap-1.5">
                                                                                ...
                                                                                {/* 返信が来た後は、過去の質問内容を履歴として表示する */}
                                                                                {hasPastNote && (
                                                                                    <div className="text-[10px] text-slate-600 bg-slate-100 p-1.5 rounded border border-slate-200">
                                                                                        <span className="font-bold text-slate-500">前回送信:</span> {item.admin_note}
                                                                                    </div>
                                                                                )}

                                                                                <div className="flex items-center gap-1">
                                                                                    {/* key に item.status を指定することで、返信が来て pending に戻った際に入力欄を自動リセット */}
                                                                                    <input 
                                                                                        key={item.status}
                                                                                        name="adminNote" 
                                                                                        defaultValue={isNoteSent ? item.admin_note : ''} 
