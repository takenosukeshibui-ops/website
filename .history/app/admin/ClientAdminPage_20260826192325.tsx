...
// Wise の請求用ディープリンク作成ボタン（emailパラメータエラー回避版）
function WiseInvoiceLinkButton({ order }: { order: any }) {
    const wiseEmail = order.profiles?.wise_email || ''
    const amount = order.total_amount || 0

    // ★ご自身の Wise ビジネスハンドル名（例: https://wise.com/pay/me/mycompany の mycompany 部分）
    const wiseHandle = 'your_actual_wise_handle'
    ...
    const handleOpenWise = () => {
        ...
        // Wiseのpay/meリンクで確実に動作するパラメータのみを指定（emailは除外）
        ...
        // メールアドレスを自動でクリップボードにコピーして案内を表示
        if (wiseEmail) {
            navigator.clipboard.writeText(wiseEmail)
            alert(`✅ ユーザーのWiseメールアドレスをクリップボードにコピーしました！\n\n宛先: ${wiseEmail}\n\nWiseの画面が開いたら、検索欄に貼り付けて（Ctrl+V）請求を作成してください。`)
        } else {
            alert('⚠️ このユーザーは Wise メールアドレスを登録していません。')
        }
    ...
    return (
        <div className="flex flex-col items-end gap-0.5">
            <button
                ...
                title={wiseEmail ? `クリックでメールアドレスをコピー: ${wiseEmail}` : 'Wiseメールアドレス未登録'}
