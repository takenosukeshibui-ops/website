...
function WiseInvoiceLinkButton({ order }: { order: any }) {
    ...
    const handleOpenWise = () => {
        ...
        // コピー用テキスト（1.メールアドレス -> 2.注文番号 -> 3.価格）
        const copyText = `${wiseEmail}\n${orderNumber}\n${amount}`

        // クリップボードへコピー
        navigator.clipboard.writeText(copyText)

        // 案内ダイアログを表示後、Wiseの請求書作成画面を開く
        alert(
...
export default function ClientAdminPage({ orders: initialOrders }: { orders: any[] }) {
    ...
    const calculateOrderStatus = (order: any) => {
        ...
        // 明示的に payment_required（請求完了）ステータスがセットされている場合
        if (order.status === 'payment_required') return 'payment_required'
    ...
    // 金額保存 ＆ 請求書送信完了処理（ステータスを payment_required へ）
        ...
        setOrders((prevOrders) =>
            prevOrders.map((order) => {
                ...
                return {
                    ...
                    status: 'payment_required', // ステータスを「決済待ち」に設定
                }
            })
        )
    ...
    // Wise等で送信完了したことを手動確認して「決済待ち」にする関数
    const handleMarkAsPaymentRequired = async (order: any) => {
        ...
        await sendInvoice(order.id, order.shipping_fee || 0, order.total_amount)
    }
                                    ...
                                    <td className="border border-slate-300 p-2">
                                        {hasInvoice ? (
                                            <div className="flex flex-col gap-1 bg-amber-50 p-1.5 rounded border border-amber-200">
                                                ...
                                                {/* Wiseで送信完了後に「決済待ちにする」ワンタップボタン */}
                                                {!isPaymentRequired && isShipped === false && (
                                                    <div className="pt-1 border-t border-amber-200/60 flex justify-end">
                                                        <button
                                                            type="button"
                                                            onClick={() => handleMarkAsPaymentRequired(order)}
                                                            className="px-2 py-0.5 bg-amber-600 hover:bg-amber-700 text-white font-bold rounded text-[11px] shadow-sm transition-colors whitespace-nowrap"
                                                        >
                                                            ✓ Wiseで送信完了 (決済待ちへ)
                                                        </button>
                                                    </div>
