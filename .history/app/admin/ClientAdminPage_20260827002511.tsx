...
function calculateInvoiceDetails(order: any, weightVal: number, hasInvoice: boolean, isEditingWeight: boolean) {
    ...
    // 5. 決済手数料の逆算
    // =========================================================
    const paymentFeeRate = 0.036; // 例: PayPal 3.6%
    const fixedPaymentFee = 40;   // 例: PayPal 40円
    ...
    // 表示の整合性を保つため、未編集中かつ保存済みならDBの総額を優先
...
export default function ClientAdminPage({ orders: initialOrders }: { orders: any[] }) {
    ...
    // ※今回は削除ボタンをUIから消しているため使用しませんが、ロジックは残しています
    const handleDeleteInvoice = async (orderId: string) => {
        if (!window.confirm('請求金額データと総重量を削除（クリア）しますか？')) return
        setOrderInputs(prev => {
            const next = { ...prev }
            delete next[orderId]
            return next
            ...
            <table className="min-w-full border-collapse border border-slate-300 text-sm text-left bg-white shadow-sm">
                <thead className="bg-slate-100">
                    <tr>
                        ...
                        <th className="border border-slate-300 p-2 min-w-[150px] text-center">総重量</th>
                ...
                <tbody>
                    {orders?.map((order) => {
                        ...
                        const isEditingWeight = inputs.weight !== '';
                        let weightVal = parseFloat(inputs.weight) || 0
                        
                        let displayWeight = inputs.weight;
                        // ローカルでの編集がなく、かつDBに保存済みの場合は送料から重量を逆算して表示を保持
                        if (!isEditingWeight && hasInvoice && order.shipping_fee !== null && order.shipping_fee !== undefined) {
                            weightVal = order.shipping_fee / 2000;
                            displayWeight = String(weightVal);
                        }
                                    ...
                                    <td className="border border-slate-300 p-2 text-center">
                                        <form action={handleInvoice.bind(null, order.id)} className="flex items-center justify-center gap-1.5">
                                            <input
                                                ...
                                                required
                                                className="border border-slate-300 p-1 rounded text-xs w-16 text-right font-mono outline-blue-400 focus:border-blue-400 transition-colors bg-white"
                                            />
                                            ...
                                            <SubmitButton pendingText="...">{hasInvoice ? '変更' : '保存'}</SubmitButton>
                                        </form>
                                    </td>

                                    {/* ★ 請求金額: 常にオレンジ色の背景で表示、削除ボタンは撤去 */}
                                        ...
                                        {isPaymentRequired || isShipped ? (
                                            ...
                                            <button
                                                ...
                                                className={`px-3 py-1 text-white rounded text-xs font-bold shadow-sm transition-colors whitespace-nowrap ${
                                                    hasInvoice ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-slate-300 cursor-not-allowed opacity-70'
                                                }`}
                                            >
                                                送信 (Wise) ↗
                                            </button>
                                        ...
                                        <form action={handleSaveTracking.bind(null, order.id)} className="flex items-center gap-1.5">
                                            ...
                                            {hasTracking && (
                                                <a
                                                    ...
                                                    title="追跡状況を確認"
                                                    className="text-blue-600 hover:text-blue-800 font-bold ml-0.5"
                                                >
                                                    ↗
                                                </a>
                                            )}
                                            <SubmitButton pendingText="...">{hasTracking ? '変更' : '保存'}</SubmitButton>
                                        </form>
                                    </td>
